...
const EnhancedNavbar: React.FC = () => {
  ...
  // Use translated statements from the translation context
  const statements = [
    t("header.greetings"),
    t("header.getAdmission"),
    t("header.januaryIntake"),
    t("header.excellence"),
  ]
      ...
      <div className="container mx-auto px-4 -mt-[10px]">
        <div className="flex items-center justify-between h-16">
          <div className="flex items-center space-x-4">
            {/* Use a container with fixed width and text truncation for consistent layout */}
            <div className="w-64 md:w-80 lg:w-96 overflow-hidden whitespace-nowrap">
          ...
          <nav className="hidden lg:flex items-center space-x-6">
            <AnimatedNavLink href="/calendar" icon={<FaCalendarAlt size={14} />}>
              <span className="text-sm font-medium truncate">{t("common.calendar")}</span>
            </AnimatedNavLink>
            <AnimatedNavLink href="https://elearning.bugemauniv.ac.ug/" icon={<RiComputerLine size={14} />}>
              <span className="text-sm font-medium truncate">{t("common.eLearning")}</span>
            </AnimatedNavLink>
            {/* <AnimatedNavLink
              href="https://erms.bugemauniv.ac.ug/buerms/default.aspx/"
              icon={<FaUserCircle size={14} />}
            >
              <span className="text-sm font-medium truncate">{t("common.staffPortal")}</span>
            </AnimatedNavLink> */}
            <DropdownNavLink
              items={[
                { label: t("common.login"), href: "https://erms.bugemauniv.ac.ug/student/login/" },
                { label: t("common.register"), href: "https://erms.bugemauniv.ac.ug/student/registration/" },
              ]}
            >
              <span className="text-sm font-medium truncate">{t("common.studentsPortal")}</span>
            </DropdownNavLink>
            <motion.a
              href="https://erms.bugemauniv.ac.ug/application/"
              target="_blank"
              rel="noopener noreferrer"
              className="bg-blue-600 text-white px-6 py-1 rounded-full text-sm font-semibold hover:bg-blue-700 transition-colors duration-300 whitespace-nowrap"
        ...
        {/* Mobile Menu */}
        <AnimatePresence>{navbarOpen && <MobileMenu closeMenu={() => setNavbarOpen(false)} />}</AnimatePresence>
      </div>
    </header>
  )
}

const AnimatedNavLink: React.FC<{ href: string; icon: React.ReactNode; children: React.ReactNode }> = ({
  href,
  icon,
  children,
}) => (
...
const DropdownNavLink: React.FC<{
  ...
  // Adjust dropdown position for RTL languages
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className={`absolute ${dropdownPosition} mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-50`}
          >
      ...
      <nav className="flex flex-col space-y-3 py-4 px-4">
        ...
        <div>
          <button className="flex items-center space-x-2 text-gray-700 hover:text-blue-600 text-sm">
            <FaGraduationCap size={14} />
            <span>{t("common.studentsPortal")}</span>
          </button>
          <div className="mt-2 pl-6">
            <a
              href="https://erms.bugemauniv.ac.ug/student/login/"
              className="block text-gray-600 hover:text-blue-600 text-xs"
            >
              {t("common.login")}
            </a>
            <a
              href="https://erms.bugemauniv.ac.ug/student/registration/"
              className="block text-gray-600 hover:text-blue-600 text-xs"
            >
              {t("common.register")}
            </a>
        ...
        <a
          href="https://erms.bugemauniv.ac.ug/application/"
          className="bg-blue-600 text-white text-sm px-4 py-2 rounded-full text-center font-semibold hover:bg-blue-700 transition-colors duration-300"
        >
          {t("common.applyNow")}
        </a>
